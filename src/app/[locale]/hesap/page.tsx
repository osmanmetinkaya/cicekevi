...
export default async function AccountPage({
  ...
  if (!isSupabaseConfigured()) {
    return (
      <div className="mx-auto max-w-md px-4 py-20 text-center">
        ...
        <p className="mt-3 text-sm text-ink-muted">{t("notConfigured")}</p>
      </div>
    );
  }
  ...
  if (!user) {
    redirect({ href: "/giris?next=/hesap", locale });
    return null;
  }
  ...
  return (
    <div className="mx-auto max-w-2xl px-4 py-10 sm:px-6">
      <div className="flex items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-2">
          {user.app_metadata?.role === "admin" && (
            ...
            >
              <LayoutDashboard size={15} /> {t("adminPanel")}
            </Link>
          )}
          <form action={signOut}>
            <button
      ...
      <h2 className="mt-10 flex items-center gap-2 font-serif text-xl text-ink">
        <Package size={19} className="text-leaf-600" /> {t("orderHistory")}
      </h2>
